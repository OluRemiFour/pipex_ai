...
interface AuditLog {
  ...
  approved: boolean;
  timestamp: string;
  details?: any; // Changed from unknown to any to prevent React error
}

export default function DashboardPage() {
  const [user, setUser] = useState<any>(null); // Changed from unknown to any
  const [repositories, setRepositories] = useState<Repository[]>([]);
  const [issues, setIssues] = useState<Issue[]>([]);
  const [pullRequests, setPullRequests] = useState<PullRequest[]>([]);
  const [auditLogs, setAuditLogs] = useState<AuditLog[]>([]);
      ...
      <div className="pt-24 md:pt-32 pb-20 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 sm:p-4">
                <GitBranch className="w-5 h-5 sm:w-6 sm:h-6 text-cyan-400 mb-2" />
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 sm:p-4">
                <CheckCircle2 className="w-5 h-5 sm:w-6 sm:h-6 text-green-400 mb-2" />
                <div className="text-xl sm:text-2xl font-bold text-white">
                  {stats.activeRepos}
                </div>
                <div className="text-xs sm:text-sm text-slate-400">
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 sm:p-4">
                <AlertTriangle className="w-5 h-5 sm:w-6 sm:h-6 text-yellow-400 mb-2" />
                <div className="text-xl sm:text-2xl font-bold text-white">
                  {stats.openIssues}
                </div>
                <div className="text-xs sm:text-sm text-slate-400">
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 sm:p-4">
                <GitPullRequest className="w-5 h-5 sm:w-6 sm:h-6 text-blue-400 mb-2" />
                <div className="text-xl sm:text-2xl font-bold text-white">
                  {stats.openPRs}
                </div>
                <div className="text-xs sm:text-sm text-slate-400">
              ...
              <div className="col-span-2 lg:col-span-1 bg-slate-800 border border-red-500/50 rounded-xl p-3 sm:p-4">
          ...
          <div className="flex gap-1 sm:gap-2 mb-6 border-b border-slate-700 overflow-x-auto pb-1">
            {["overview", "repos", "issues", "prs", "audit"].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab as any)}
          ...
          >
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 sm:p-6">
                  ...
                  {issues.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-center">
                      <FileQuestion className="w-12 h-12 sm:w-16 sm:h-16 text-slate-600 mb-4" />
                    ...
                    <div className="space-y-3">
                      {issues.slice(0, 5).map((issue) => (
                        <div
                          key={issue._id}
                          className="flex flex-col sm:flex-row sm:items-start gap-3 p-3 bg-slate-900 rounded-lg"
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`px-2 py-1 rounded text-xs font-semibold border ${getSeverityColor(
                  ...
                  <div className="space-y-3">
                    {pullRequests.slice(0, 5).map((pr) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <GitPullRequest
                            className={`w-4 h-4 sm:w-5 sm:h-5 ${getStatusColor(
                              pr.status
                            )}`}
                          />
                          <div className="sm:hidden text-xs text-slate-500">
                        ...
                        <div className="flex-1">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-white font-medium text-sm sm:text-base hover:text-cyan-400 inline-flex items-center gap-1"
                          >
                            #{pr.prNumber} {pr.title}
                        ...
                        <div className="flex justify-between sm:flex-col items-end gap-2">
                          <div
                            className={`text-xs font-semibold ${getStatusColor(
                              pr.status
                            )}`}
            ...
            {activeTab === "repos" && (
              <div className="space-y-4">
                {repositories.length === 0 ? (
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-8 sm:p-12">
                    <div className="flex flex-col items-center justify-center text-center">
                      <GitBranch className="w-16 h-16 sm:w-20 sm:h-20 text-slate-600 mb-4" />
                      ...
                      {!githubConnected ? (
                        <button
                          onClick={handleConnectGitHub}
                          disabled={connecting}
                          className="px-4 sm:px-6 py-2 sm:py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50 text-sm sm:text-base"
                        >
                          {connecting ? (
                        ...
                        <button
                          onClick={handleSyncGitHub}
                          disabled={syncing}
                          className="px-4 sm:px-6 py-2 sm:py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50 text-sm sm:text-base"
                        >
                          {syncing ? (
                            <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" />
                          ) : (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                          <button
                            ...
                            disabled={analyzing === repo._id}
                            className="px-3 sm:px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-xs sm:text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto justify-center"
                          >
                            {analyzing === repo._id ? (
                              <>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-1">
                      <input
                        ...
                        value={issueSearch}
                        onChange={(e) => setIssueSearch(e.target.value)}
                        className="w-full px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 text-sm sm:text-base"
                      />
                    </div>
                    <select
                      value={issueSeverityFilter}
                      onChange={(e) => setIssueSeverityFilter(e.target.value)}
                      className="px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-cyan-500 text-sm sm:text-base"
                    >
                      <option value="all">All Severities</option>
                      <option value="CRITICAL">Critical</option>
                      <option value="HIGH">High</option>
                ...
                {filteredIssues.length === 0 ? (
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-8 sm:p-12">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-slate-400 mb-6 max-w-md text-sm sm:text-base">
                        {issues.length === 0
                          ? "Analyze your repositories to detect bugs, performance issues, and code quality problems"
                          : "Try adjusting your search or filter criteria"}
                      </p>
                      <button
                        onClick={() => setActiveTab("repos")}
                        className="px-4 sm:px-6 py-2 sm:py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg transition-colors flex items-center gap-2 text-sm sm:text-base"
                      >
                              ...
                              <div className="mt-4 pt-4 border-t border-slate-700 space-y-3">
                                ...
                                {issue.status === "detected" && (
                                  <button
                                    ...
                                    disabled={fixingIssue === issue._id}
                                    className="mt-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed w-full justify-center"
                                  >
                                    {fixingIssue === issue._id ? (
                                      <>
                                        <Loader2 className="w-4 h-4 animate-spin" />
                          ...
                          <div className="flex flex-col gap-3 items-start sm:items-end">
                            ...
                            {issue.status === "detected" && (
                              <button
                                ...
                                disabled={fixingIssue === issue._id}
                                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto justify-center"
                              >
                                {fixingIssue === issue._id ? (
                                  <>
                                    <Loader2 className="w-4 h-4 animate-spin" />
            ...
            {activeTab === "prs" && (
              <div className="space-y-4">
                {pullRequests.length === 0 ? (
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-8 sm:p-12">
                    <div className="flex flex-col items-center justify-center text-center">
                      <GitPullRequest className="w-16 h-16 sm:w-20 sm:h-20 text-slate-600 mb-4" />
                      <h3 className="text-lg sm:text-xl font-bold text-white mb-2">
                        No Pull Requests Yet
                      </h3>
                      <p className="text-slate-400 mb-6 max-w-md text-sm sm:text-base">
                      ...
                      <button
                        onClick={() => setActiveTab("issues")}
                        className="px-4 sm:px-6 py-2 sm:py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-colors flex items-center gap-2 text-sm sm:text-base"
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-lg sm:text-xl font-bold text-white hover:text-cyan-400 inline-flex items-center gap-1"
                            >
                              #{pr.prNumber}
                              <ExternalLink className="w-3 h-3" />
                            </a>
            ...
            {activeTab === "audit" && (
              <div className="space-y-3">
                {auditLogs.length === 0 ? (
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-8 sm:p-12">
                    <div className="flex flex-col items-center justify-center text-center">
                      <Activity className="w-16 h-16 sm:w-20 sm:h-20 text-slate-600 mb-4" />
                      <h3 className="text-lg sm:text-xl font-bold text-white mb-2">
                        No Audit Logs
                      </h3>
                  ...
                  auditLogs.map((log) => (
                    <div
                      key={log._id}
                      className="bg-slate-800 border border-slate-700 rounded-xl p-4 sm:p-6 hover:border-slate-600 transition-colors"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        <div className="flex items-start gap-4 flex-1">
                          <div
                          ...
                          <div className="flex-1">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                              ...
                              <div
                                className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold ${getSeverityColor(
                                  log.riskLevel
                                )}`}
                              >
                                {log.riskLevel} Risk
                            ...
                            <div className="space-y-3">
                              ...
                              {expandedAuditLog === log._id && (
                                <div className="bg-slate-900 rounded-lg p-4">
                                  ...
                                  <pre className="text-xs sm:text-sm text-slate-300 font-mono whitespace-pre-wrap break-words">
